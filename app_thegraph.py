...
def _fetch_order_filled_events(
...
def _kpis_from_fills(rows: List[Dict[str, object]], token_ids: List[str]) -> Dict[str, object]:
...
def main() -> None:
    ...
    print(f"The Graph endpoint: {graph.query_url}")
    ...
    for idx, t in enumerate(targets, start=1):
        ...
        try:
            rows = _fetch_order_filled_events(
                graph=graph,
                token_ids=token_ids,
                first=page_size,
                max_pages=max_pages,
